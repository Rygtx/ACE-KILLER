...
class MainWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        self.brute_mode_checkbox.setToolTip(
            "深度清理模式会使用Windows系统API清理所有进程的工作集，效率更高但更激进；\n"
        ...
        self.memory_info_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        self.config_info_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        self.clean_stats_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        self.stop_service_btn.setToolTip(
            "停止ACE-GAME、ACE-BASE、AntiCheatExpert Service、AntiCheatExpert Protection服务"
        )
        ...
        self.delete_service_btn.setToolTip(
            "删除ACE-GAME、ACE-BASE、AntiCheatExpert Service、AntiCheatExpert Protection服务"
        )
    ...
    def setup_tray(self):
        ...
        if self.icon_path:
            ...
            self.tray_icon.setIcon(QIcon())  # 使用默认图标
    ...
    def get_status_html(self):
        ...
        if ace_running and self.monitor.anticheat_killed:
            html.append(
                '<p class="status-item">✅ ACE-Tray进程: <span class="status-success">已被终止</span>  (反作弊安装弹窗进程)</p>'
            )
        elif ace_running:
            html.append(
                '<p class="status-item">🔄 ACE-Tray进程: <span class="status-warning">正在处理</span>  (反作弊安装弹窗进程)</p>'
            )
        else:
            html.append(
                '<p class="status-item">ℹ️ ACE-Tray进程: <span class="status-normal">未处理</span>  (反作弊安装弹窗进程)</p>'
            )
        ...
        if scan_running:
            ...
            if self.monitor.scanprocess_optimized:
                html.append(
                    '<p class="status-item">✅ SGuard64进程: <span class="status-success">已被优化</span>  (反作弊扫盘进程)</p>'
                )
            else:
                html.append(
                    '<p class="status-item">🔄 SGuard64进程: <span class="status-warning">正在运行 (未优化)</span>  (反作弊扫盘进程)</p>'
                )
        else:
            html.append(
                '<p class="status-item">⚠️ SGuard64进程: <span class="status-error">未在运行</span>  (反作弊扫盘进程)</p>'
            )
        html.append("</div>")
        ...
        for service_name, service_info in service_results.items():
            ...
            if service_exists:
                if status == "running":
                    html.append(
                        f'<p class="status-item">✅ {service_name}: <span class="status-success">正在运行</span></p>'
                    )
                elif status == "stopped":
                    html.append(
                        f'<p class="status-item">⚠️ {service_name}: <span class="status-error">已停止</span></p>'
                    )
                else:
                    html.append(
                        f'<p class="status-item">ℹ️ {service_name}: <span class="status-normal">{status}</span></p>'
                    )

                # 服务启动类型
                if start_type == "auto":
                    html.append(
                        f'<p class="status-item">⚙️ {service_name}启动类型: <span class="status-success">自动启动</span></p>'
                    )
                elif start_type == "disabled":
                    html.append(
                        f'<p class="status-item">⚙️ {service_name}启动类型: <span class="status-error">已禁用</span></p>'
                    )
                elif start_type == "manual":
                    html.append(
                        f'<p class="status-item">⚙️ {service_name}启动类型: <span class="status-normal">手动</span></p>'
                    )
                else:
                    html.append(
                        f'<p class="status-item">⚙️ {service_name}启动类型: <span class="status-normal">{start_type}</span></p>'
                    )
            else:
                html.append(
                    f'<p class="status-item">❓ {service_name}: <span class="status-disabled">未找到</span></p>'
                )
        ...
        if self.memory_cleaner.running:
            ...
            if mem_info:
                ...
                html.append(
                    f'<p class="status-item">🍋‍🟩 内存使用: <span class="{status_class}">{used_percent:.1f}%</span> ({used_gb:.1f}GB / {total_gb:.1f}GB)</p>'
                )

                # 添加自定义清理配置信息
                html.append(
                    f'<p class="status-item">⏱️ 清理间隔: <span class="status-normal">{self.memory_cleaner.clean_interval}秒</span></p>'
                )
                html.append(
                    f'<p class="status-item">📊 触发阈值: <span class="status-normal">{self.memory_cleaner.threshold}%</span></p>'
                )
                html.append(
                    f'<p class="status-item">⏲️ 冷却时间: <span class="status-normal">{self.memory_cleaner.cooldown_time}秒</span></p>'
                )
                ...
                if cache_info:
                    ...
                    html.append(
                        f'<p class="status-item">💾 系统缓存: <span class="status-normal">{cache_size:.1f}GB</span> (峰值: {peak_size:.1f}GB)</p>'
                    )
            else:
        ...
        html.append(
            f'<p class="status-item">🔔 通知功能: <span class="{notification_class}" style="font-weight: bold;">{notification_text}</span></p>'
        )
        ...
        html.append(
            f'<p class="status-item">🔁 开机自启: <span class="{autostart_class}" style="font-weight: bold;">{autostart_text}</span></p>'
        )
        ...
        html.append(
            f'<p class="status-item">🪟 关闭行为: <span class="{close_behavior_class}" style="font-weight: bold;">{close_behavior_text}</span></p>'
        )
        ...
        html.append(
            f'<p class="status-item">🐛 调试模式: <span class="{debug_class}" style="font-weight: bold;">{debug_text}</span></p>'
        )

        # 主题状态
        html.append(
            f'<p class="status-item">🎨 当前主题: <span class="status-normal" style="font-weight: bold;">{self._get_theme_display_name()}</span></p>'
        )
    ...
    def update_memory_status(self):
        ...
        if cache_info:
            ...
            self.cache_info_label.setText(
                f"系统缓存: 当前 {cache_size_gb:.1f}GB ({cache_percent:.1f}%) | 峰值 {cache_peak_gb:.1f}GB"
            )
        ...
        # 更新配置信息标签
        config_text = (
            f"配置: 清理间隔 {self.memory_cleaner.clean_interval}秒 | "
        ...
        stats_text = (
            f"累计释放: {stats['total_cleaned_mb']:.2f}MB | "
    ...
    def _toggle_notifications(self, from_tray=False):
        ...
        if self.monitor.config_manager.save_config():
            logger.debug(
                f"通知状态已更改并保存: {'开启' if self.monitor.config_manager.show_notifications else '关闭'}"
            )
        else:
            logger.warning(
                f"通知状态已更改但保存失败: {'开启' if self.monitor.config_manager.show_notifications else '关闭'}"
            )
    ...
    def _toggle_auto_start(self, from_tray=False):
        ...
        if self.monitor.config_manager.save_config():
            ...
            logger.warning(
                f"开机自启状态已更改但保存失败: {'开启' if self.monitor.config_manager.auto_start else '关闭'}"
            )
    ...
    def delete_ace_services(self):
        ...
        # 服务列表
        services = ["ACE-GAME", "ACE-BASE", "AntiCheatExpert Service", "AntiCheatExpert Protection"]
    ...
    def optimize_anticheat_processes(self):
        ...
        for i, process_name in enumerate(anticheat_processes):
            ...
            success_count, count = io_manager.set_process_io_priority_by_name(
                process_name, IO_PRIORITY_HINT.IoPriorityVeryLow, PERFORMANCE_MODE.ECO_MODE
            )
            ...
            if count > 0:
                ...
                # 将成功优化的进程添加到自动优化列表
                if success_count > 0:
                    self._add_to_auto_optimize_list(
                        process_name, PERFORMANCE_MODE.ECO_MODE, added_to_list, updated_in_list
                    )
    ...
    def _add_to_auto_optimize_list(
        self, process_name: str, performance_mode: int, added_list: list, updated_list: list
    ):
        """将进程添加到自动优化列表"""
        # 导入性能模式枚举
        from utils.process_io_priority import PERFORMANCE_MODE
        ...
        if not existing_found:
            ...
            self.monitor.config_manager.io_priority_processes.append(
                {"name": process_name, "performance_mode": performance_mode, "added_time": time.time()}
            )
    ...
    def show_auto_optimize_tab(self):
        """显示自动优化列表选项卡"""
        # 导入对话框类
        from ui.process_io_priority_manager import ProcessIoPriorityManagerWidget

        # 创建对话框
        widget = ProcessIoPriorityManagerWidget(self, self.monitor.config_manager)
    ...
    def start_ace_program(self):
        ...
        try:
            ...
            # 执行命令启动反作弊程序
            subprocess.Popen([ace_path, "enable"], shell=False, creationflags=subprocess.CREATE_NO_WINDOW)

            logger.debug("已执行ACE反作弊程序启动命令")

            # 显示成功消息
            QMessageBox.information(self, "启动命令已执行", "ACE反作弊程序启动命令已执行！\n请重新启动电脑才能生效。")

            # 发送通知
            if self.monitor.config_manager.show_notifications:
                send_notification(title="ACE-KILLER", message="ACE反作弊程序启动命令已执行", icon_path=self.icon_path)
    ...
    def uninstall_ace_program(self):
        ...
        try:
            ...
            if not os.path.exists(uninstaller_path):
                QMessageBox.warning(self, "卸载失败", "未找到ACE反作弊卸载程序，请确认已安装ACE反作弊。\n")
                return

            # 执行卸载命令
            subprocess.Popen([uninstaller_path], shell=False, creationflags=subprocess.CREATE_NO_WINDOW)

            logger.debug("已执行ACE反作弊程序卸载命令")

            # 发送通知
            if self.monitor.config_manager.show_notifications:
                send_notification(title="ACE-KILLER", message="ACE反作弊程序卸载命令已执行。", icon_path=self.icon_path)
