...
class ThemeManager(QObject):
    ...
    def get_stylesheet(self, theme: str | None = None) -> str:
        """获取指定主题的样式表"""
        if theme is None:
            theme = self._current_theme

        if theme == "dark":
            return self._dark_stylesheet or ""
        else:
            return self._light_stylesheet or ""

    def is_dark_theme(self, theme: str | None = None) -> bool:
        """判断是否为深色主题"""
        if theme is None:
            theme = self._current_theme
        return theme == "dark"
...
class StatusHTMLGenerator:
    ...
    @staticmethod
    def get_html_style(theme: str | None = None) -> str:
        """获取状态HTML的CSS样式"""
        if theme is None:
