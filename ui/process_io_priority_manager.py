            ...
            for i, proc in enumerate(all_processes):
                ...
                try:
                    ...
                    proc_info = proc.as_dict(
                        attrs=["pid", "name", "username", "status", "create_time", "memory_percent"]
                    )
...
class ProcessIoPriorityManagerWidget(QWidget):
    """进程I/O优先级管理对话框"""
    ...
    def setup_ui(self):
        ...
        # 创建自定义标题栏（只有关闭按钮）
        self.title_bar = CustomTitleBar(self, show_systray=False, show_minimize=False, show_close=True)
    ...
    def create_process_tab(self):
        ...
        header.setSectionResizeMode(2, QHeaderView.ResizeMode.Interactive)  # 用户
        ...
        header.setSectionResizeMode(5, QHeaderView.ResizeMode.Interactive)  # 创建时间
        header.setSectionResizeMode(6, QHeaderView.ResizeMode.Interactive)  # 性能模式
    ...
    def create_auto_optimize_tab(self):
        ...
        self.auto_optimize_table.setHorizontalHeaderLabels(["📋 进程名", "⚙️ 性能模式", "🕐 添加时间", "🛠️ 操作"])
        ...
        auto_header.setSectionResizeMode(0, QHeaderView.ResizeMode.Stretch)  # 进程名
        auto_header.setSectionResizeMode(1, QHeaderView.ResizeMode.Interactive)  # 性能模式
        auto_header.setSectionResizeMode(2, QHeaderView.ResizeMode.Interactive)  # 添加时间
        auto_header.setSectionResizeMode(3, QHeaderView.ResizeMode.Fixed)  # 操作
    ...
    def _apply_filters(self):
        ...
        # 更新统计信息
        self.process_count_label.setText(f"显示进程数: {len(filtered_processes)} / 总进程数: {len(self.all_processes)}")
    ...
    def _populate_row(self, row, proc):
        ...
        pid_item.setData(Qt.ItemDataRole.UserRole, proc)  # 存储完整进程信息
        ...
        try:
            create_time = time.strftime("%m-%d %H:%M", time.localtime(proc.get("create_time", 0)))
        except:
            create_time = "N/A"
        ...
        if not action_widget:
            ...
            if layout and layout.itemAt(0):
                apply_btn = layout.itemAt(0).widget()
                if apply_btn:
                    apply_btn.setProperty("process_info", proc)
    ...
    def apply_performance_mode_by_button(self, button):
        ...
        # 获取选择的性能模式
        performance_mode_combo = self.process_table.cellWidget(row, 6)
        if not performance_mode_combo or not hasattr(performance_mode_combo, "currentData"):
            return
        ...
        if not success:
            QMessageBox.warning(
                self, "优化失败", f"无法优化进程 {process_name} (PID: {pid})\n可能是权限不足或进程已退出"
            )
            return
        ...
        if self.config_manager.save_config():
            ...
            QMessageBox.warning(self, "保存失败", f"进程优化成功，但无法保存到自动优化列表\n请检查程序权限")
    ...
    def load_auto_optimize_list(self):
        ...
        for row, proc in enumerate(processes):
            ...
            if not performance_combo:
                ...
                performance_combo.currentIndexChanged.connect(
                    lambda index, combo=performance_combo: self.on_auto_performance_mode_changed(combo)
                )
            ...
            if not action_widget:
                ...
                delete_btn.clicked.connect(
                    lambda checked, btn=delete_btn: self.delete_from_auto_optimize_list_by_button(btn)
                )
    ...
    def on_auto_optimize_table_section_resized(self, logical_index, old_size, new_size):
        """处理自动优化表格列宽调整，限制最小宽度"""
        # 定义每列的最小宽度
        min_widths = {0: 120, 1: 120, 2: 120, 3: 100}  # 进程名  # 性能模式  # 添加时间  # 操作
            ...
            if proc.get("name") == process_name:
                ...
                if old_performance_mode != new_performance_mode:
                    ...
                    if self.config_manager.save_config():
                        ...
                        logger.debug(
                            f"更新自动优化进程 {process_name} 的性能模式: {old_performance_mode} -> {new_performance_mode}"
                        )
                    else:
                        # 保存失败，恢复原来的值
    ...
    def _apply_to_running_process(self, process_name, performance_mode):
        ...
        try:
            ...
            success_count, total_count = self.io_manager.set_process_io_priority_by_name(
                process_name, priority=None, performance_mode=performance_mode  # 自动确定优先级
            )

            if total_count > 0:
                if success_count == total_count:
                    logger.debug(
                        f"已将性能模式 {performance_mode} 应用到所有运行中的 {process_name} 进程 ({success_count}/{total_count})"
                    )
                else:
...
def show_process_io_priority_manager(parent=None, config_manager=None):
    """显示进程I/O优先级管理对话框"""
    widget = ProcessIoPriorityManagerWidget(parent, config_manager)
    return widget.show()
