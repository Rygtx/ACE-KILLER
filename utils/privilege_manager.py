...
class WindowsPrivilegeManager:
    ...
    def _init_privileges(self):
        ...
        try:
            ...
            self.available_functions = {
                ...
                "debug_other_processes": win32security.SE_DEBUG_NAME in privilege_details and 
                                        privilege_details[win32security.SE_DEBUG_NAME]["success"],
                "set_process_io_priority": (privilege_status["enhanced"]["acquired"] > 0 or 
                                          privilege_status["process"]["acquired"] > 0),
                "set_process_priority": (privilege_status["enhanced"]["acquired"] > 0 or 
                                       privilege_status["process"]["acquired"] > 0),
            }
    ...
    def log_privilege_status(self):
        ...
        if summary["privilege_status"]:
            ...
            logger.info(f"  权限统计: 核心({ps['core']['acquired']}/{ps['core']['total']}) "
